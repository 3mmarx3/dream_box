...
function moveBack() {
  ...
  document.querySelectorAll('.content-panel').forEach(step => step.classList.remove('visible'));
...
function completeProcess() {
  const lastStep = document.getElementById('step3');
  if (lastStep) lastStep.classList.remove('visible');
...
const downloadTimer = setInterval(function() {
  if (timeLeft <= 0) {
    ...
    const targetUrl = finalLinkElement ? finalLinkElement.getAttribute('href') : "https://www.google.com";
